...
The firmware stores up to 64 unsent non-heartbeat events in RAM. Event payloads are absolute state, so a newer event from a physical source replaces the older buffered event carrying the same `source_id` and moves to the back of the queue; replayed sequence IDs therefore still ascend, but a replay can skip sequence IDs that were superseded while offline. System-source events (`fault_detected`, `sync_requested`) are not coalesced, and a full queue rejects them with a logged loss. Application checks retry opportunities at least once per one-second receive timeout, so a missed MQTT connection notification or temporarily full MQTT outbox does not leave the FIFO waiting forever for another reconnect. An older FIFO entry must be accepted before a newer event. ESP-MQTT has a separate 16 KiB RAM outbox; accepted QoS 1 messages can be retransmitted by the library and may expire under its default policy.
