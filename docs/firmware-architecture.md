...
| `OfflineEventStore` | Mutex-protected, bounded `std::deque<Event>` FIFO, capacity 64; drop-new policy |
...
The FIFO retries oldest first on connection notification, on normal loop iterations, and before new publications. A head event is removed only after MQTT accepts it. MQTT acceptance is not broker PUBACK or Odoo acknowledgement. Heartbeats that cannot be sent are discarded. There is no persistent event storage: both the FIFO and MQTT outbox disappear on reboot. NVS initialization supports ESP-IDF services, not a persistent application event queue.
